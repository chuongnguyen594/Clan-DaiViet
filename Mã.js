...
function doGet(e) {
  ...
  if (e && e.parameter.action) {
    ...
    switch (e.parameter.action) {
      ...
      case "getDanhSachBayCup2":
        data = getBayCupHuyenThoai2();
        break;

      case "BayCupHuyenThoai1":
      case "getDanhSachBayCup1":
        data = getBayCupHuyenThoai1();
        break;
...
function get3vs3LichThiDau() {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  return ss.getSheetByName("3vs3_LichThiDau").getDataRange().getDisplayValues();
}

function get3vs3BangDiem() {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  return ss.getSheetByName("3vs3_BangDiem").getDataRange().getDisplayValues();
}

function get3vs3TopTeam3Sao() {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  return ss.getSheetByName("3vs3_TopTeam3Sao").getDataRange().getDisplayValues();
}

function get3vs3TopTranDau() {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  return ss.getSheetByName("3vs3_TopTranDau").getDataRange().getDisplayValues();
}

function get3vs3TopCaNhan() {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  return ss.getSheetByName("3vs3_TopCaNhan").getDataRange().getDisplayValues();
}

function get3vs3TopThu1Sao() {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  return ss.getSheetByName("3vs3_TopThu1Sao").getDataRange().getDisplayValues();
}
...
function bocTham3vs3() {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  const ds = ss.getSheetByName("3vs3Team").getRange(2, 2, 16, 1).getValues().flat();
...
function dangKy3vs3(e) {
  ...
  for (let i = 0; i < data.length; i++) {
    if (data[i][0].toLowerCase() == team.toLowerCase()) {
      return { status: "error", message: "Tên đội đã tồn tại!" };
    }
    if (data[i][1].toLowerCase() == captain.toLowerCase()) {
      return { status: "error", message: "Đội trưởng đã đăng ký!" };
    }
  }

  sheet.appendRow([sheet.getLastRow(), team, captain, tv2, tv3]);
...
function getDanhSachTeam3vs3() {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  const sheet = ss.getSheetByName("3vs3Team");
  return sheet.getRange(2, 2, Math.max(sheet.getLastRow() - 1, 0), 1).getDisplayValues();
}

function getMediaCenter() {
  const sh = SpreadsheetApp.openById(SHEET_ID).getSheetByName("MEDIA_CENTER");
...
function dangKyBayCupHuyenThoai2(zalo, danhSachAcc) {
  ...
  if (!sheet) {
    ...
    sheet.appendRow(["STT", "Zalo", "Nick game", "Mã code", "Đã đóng tiền", "Lên hạng"]);
  }

  if (!zalo || !danhSachAcc || danhSachAcc.length === 0) {
    return { status: "error", message: "Thiếu thông tin đăng ký!" };
  }
...
function getBayCupHuyenThoai2() {
  ...
  // Lấy toàn bộ từ dòng 1 (gồm cả tiêu đề) đểFrontend hiển thị đúng cột
  return sheet.getDataRange().getDisplayValues();
}
...
function getBayCupHuyenThoai1() {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  const sheet = ss.getSheetByName("BayCupHuyenThoai1");
...
function DangKyBayCupHuyenThoai1(e) {
  ...
  if (!sheet) {
    return { status: "error", message: "Không tìm thấy sheet BayCupHuyenThoai1!" };
  }
  ...
  for (let i = 1; i <= soAcc; i++) {
    ...
    if (!nick || !code) {
      return { status: "error", message: "Thiếu Nick hoặc Mã code acc " + i };
    }
